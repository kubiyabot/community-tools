...
deployment_management_tool = KubernetesTool(
    ...
    if ! kubectl "${action}" deployment "${name}" ${image_flag} ${replicas_flag} ${namespace_flag}; then
        echo "❌ Failed to ${action} deployment ${name} in namespace ${namespace}"
        exit 1
    fi

    echo "✅ Successfully ${action}d deployment ${name} in namespace ${namespace}"
    ''',
    args=[
