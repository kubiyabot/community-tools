...
pr_list = GitHubRepolessCliTool(
    ...
    content="gh search prs --json number,title,url,state,author,updatedAt,repository $([[ -n \"$repo\" ]] && echo \"--repo $repo\") $([[ -n \"$state\" ]] && echo \"--state $state\") $([[ -n \"$limit\" ]] && echo \"--limit $limit\") $([[ -n \"$author\" ]] && echo \"--author $author\") $([[ -n \"$assignee\" ]] && echo \"--assignee $assignee\") $([[ -n \"$org\" ]] && echo \"--owner $org\")",
    args=[
...
pr_review = GitHubCliTool(
    ...
    content="gh pr review --repo \"$repo\" $number --$review_type --body \"$body\"",
    args=[
        Arg(name="repo", type="str", description="Repository name in 'owner/repo' format. Example: 'octocat/Hello-World'", required=True),
        Arg(name="number", type="int", description="Pull request number. Example: 123", required=True),
