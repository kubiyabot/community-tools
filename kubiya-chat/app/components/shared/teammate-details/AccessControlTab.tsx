          ...
          <section className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <Badge variant="secondary" className="bg-[#1E293B] text-[#94A3B8] border-[#1E293B]">
                {teammate.allowed_groups.length} {teammate.allowed_groups.length === 1 ? 'Group' : 'Groups'}
              </Badge>
            </div>
            <div className="grid gap-4">
              {teammate.allowed_groups.map((groupId) => {
                const group = getEntityMetadata(groupId);
                return (
                  <div key={groupId} className="p-6 rounded-lg bg-[#1E293B] border border-[#1E293B] hover:border-[#7C3AED]/20 transition-all duration-200">
        ...
        {teammate.allowed_users && teammate.allowed_users.length > 0 && (
          <section className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <Badge variant="secondary" className="bg-[#1E293B] text-[#94A3B8] border-[#1E293B]">
                {teammate.allowed_users.length} {teammate.allowed_users.length === 1 ? 'User' : 'Users'}
              </Badge>
            </div>
            <div className="grid gap-4">
              {teammate.allowed_users.map((userId) => {
                const user = getEntityMetadata(userId);
                return (
                  <div key={userId} className="p-6 rounded-lg bg-[#1E293B] border border-[#1E293B] hover:border-[#7C3AED]/20 transition-all duration-200">
                    <div className="flex items-start gap-4">
                      <div className="p-2.5 rounded-md bg-[#7C3AED]/10">
                        {user?.image ? (
