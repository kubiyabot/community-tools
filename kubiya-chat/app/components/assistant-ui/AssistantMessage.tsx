...
import { Bot, AlertTriangle, Copy, Check, Terminal, Loader2, ChevronDown, Code, Clock } from 'lucide-react';
...
import { WebhookSuccessMessage } from './WebhookSuccessMessage';
...
const AssistantMessageComponent = ({ message, isSystem, isTool, sourceMetadata }: AssistantMessageProps) => {
  ...
  const isScheduledTask = message.metadata?.custom?.isScheduledTask && message.metadata?.custom?.task;
  const isWebhook = message.metadata?.custom?.isWebhook && message.metadata?.custom?.webhook;
  ...
  if (isWebhook) {
    return <WebhookSuccessMessage webhook={message.metadata.custom.webhook as Webhook} />;
  }

  if (isScheduledTask) {
    return <ScheduledTaskMessage task={message.metadata.custom.task as ScheduledTask} />;
  }
    ...
    return (
      <div className="flex items-start gap-3">
        <div className={cn(
          ...
          isScheduledTask ? "bg-emerald-950/50" : "bg-amber-950/50"
        )}>
          <div className="flex items-center gap-2 mb-3">
            <div className={cn(
              "flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center",
              isScheduledTask ? "bg-emerald-100" : "bg-amber-100"
            )}>
              {isScheduledTask ? (
            ...
            <div className={cn(
              "text-sm font-medium",
              isScheduledTask ? "text-emerald-200" : "text-amber-200"
            )}>
              {isScheduledTask ? "Task Scheduled" : "System Messages"}
            </div>
          </div>
          <div className="space-y-3">
            {warnings.map((warning: string, index: number) => (
              <div 
                key={index} 
                className={cn(
                  "flex items-start gap-2 text-sm p-3 rounded group relative",
                  isScheduledTask 
              ...
              >
                <span className={cn(
                  "mt-1",
                  isScheduledTask ? "text-emerald-400" : "text-amber-400"
                )}>•</span>
