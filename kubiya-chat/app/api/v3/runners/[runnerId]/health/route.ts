...
export async function GET(
  ...
  try {
    ...
    const response = await fetch(`${apiUrl}/api/v3/runners/${params.runnerId}/health`, {
      headers: createApiHeaders(session)
    });

    if (!response.ok) {
      throw new Error(`Failed to fetch runner health: ${response.statusText}`);
    }
    ...
    return handleApiError(error, 'Failed to fetch runner health');
  }
} 