    ...
    def __init__(self, name, description, content, args=None, image="alpine:latest"):
        ...
        helper_functions = """
            ...
            validate_observe_auth() {
                if [ -z "$OBSERVE_CUSTOMER_ID" ] || [ -z "$OBSERVE_API_TOKEN" ]; then
                    echo "Error: OBSERVE_CUSTOMER_ID and OBSERVE_API_TOKEN environment variables are required"
                    exit 1
                fi
            }
            ...
            query_dataset() {
                ...
                curl -s -X POST "https://api.observeinc.com/v1/customers/$OBSERVE_CUSTOMER_ID/datasets/$dataset_id/query" \
                    -H "Authorization: Bearer $OBSERVE_API_TOKEN" \
        ...
        super().__init__(
            ...
            type="docker",
            secrets=["OBSERVE_API_TOKEN", "OBSERVE_CUSTOMER_ID"],
            env=[]
        )
